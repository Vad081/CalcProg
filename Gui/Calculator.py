...
from .Ui.CalcMain import Ui_Calculator
from .weather import WeatherWidget,owm
...
class CalculatorWindow(QtWidgets.QMainWindow, Ui_Calculator):
    ...
    def __init__(self):
        ...
        self.menuBar.addAction(self.actionWeather)

        self.actionWeather.triggered.connect(self.getWeatherResult)
    ...
    def digitClicked(self, digit):
        ...
        if ((self.pushButton_plus.isChecked() or 
            ...
            newLabel = format(float(digit.text()), '.15g')
            self._userIsTypingSecondNumber = True
        elif (('.' in self.label.text()) and digit.text() == '0'):
            newLabel = format(self.label.text() + digit.text(),'.15')
        elif not self.label.text().isdigit():
            newLabel = format(digit.text(), '.15')
        else:
            newLabel = format(float(self.label.text() + digit.text()), '.15g')
        self.outputLabel(newLabel)


    def keyPressEvent(self, digit):
        '''Команды на клавиатуре'''

        if digit.key() == Qt.Key_Return:
            self.equalsClicked()
        if digit.key() == Qt.Key_Space:
            self.clearClicked()
        if digit.key() == Qt.Key_Escape:
            self.close()


    def getWeatherResult(self):
    ...
    def rdClicked(self):
        '''Случайное число'''
        if (not self.lineEdit_input1.text()) and (not self.lineEdit_input2.text()):
